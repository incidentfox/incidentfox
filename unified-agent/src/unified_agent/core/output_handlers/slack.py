    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt < max_retries and (is_rate_limit or is_server_error):
                ...
                logger.warning(
                    "Slack API retry attempt %d/%d (delay=%.1fs, error=%s)",
...
class SlackOutputHandler(OutputHandler):
    ...
    async def post_initial(
        ...
        if not channel_id:
            logger.error("Slack output missing channel_id (config keys: %s)", list(config.keys()))
            return None
        ...
        try:
            ...
            async def _post():
                return await client.chat_postMessage(
                    channel=channel_id,
                    thread_ts=thread_ts,
                    text=f"{mention}\U0001f98a {agent_name} is working on it...",
                    blocks=blocks,
                )
            ...
            logger.info("Slack initial message posted (channel=%s, ts=%s)", channel_id, message_ts)

            return message_ts

        except Exception as e:
            logger.error("Slack initial post failed (channel=%s): %s", channel_id, e, exc_info=True)
            return None
    ...
    async def update_progress(
        ...
        try:
            ...
            logger.warning("Slack progress update failed (channel=%s): %s", channel_id, e)
    ...
    async def post_final(
        ...
        channel_id = config.get("channel_id")
        if not channel_id:
            logger.error("Slack final post missing channel_id (config keys: %s)", list(config.keys()))
            return OutputResult(
                success=False,
        ...
        try:
            ...
            logger.info("Slack final message posted (channel=%s, success=%s)", channel_id, success)
            ...
            logger.error("Slack final post failed (channel=%s): %s", channel_id, e, exc_info=True)
            return OutputResult(
                success=False,
                destination_type="slack",
                error=str(e),
