...
class OutputHandlerRegistry:
    ...
    def register(self, handler: OutputHandler) -> None:
        ...
        logger.debug("Output handler registered: %s", handler.destination_type)
...
def _register_default_handlers(registry: OutputHandlerRegistry) -> None:
    ...
    try:
        ...
        logger.debug("Google Chat handler not available: SDK not installed")
    ...
    try:
        ...
        logger.debug("Teams handler not available: SDK not installed")
...
async def post_to_destinations(
    ...
    *,
    ...
    """
    if not destinations:
        logger.debug("No output destinations configured")
        return []
    ...
    for dest in destinations:
        handler = registry.get(dest.type)
        if not handler:
            logger.warning("Unknown output destination type: %s", dest.type)
            results.append(
                OutputResult(
                    success=False,
        ...
        try:
            ...
            logger.info(
                "Output posted to %s (success=%s)", dest.type, result.success
            )

        except Exception as e:
            logger.error("Output post failed for %s: %s", dest.type, e)
            results.append(
                OutputResult(
                    success=False,
                    destination_type=dest.type,
...
async def post_initial_to_destinations(
    ...
    for dest in destinations:
        ...
        try:
            ...
            logger.warning("Initial post failed for %s: %s", dest.type, e)
