...
# Self-signed TLS cert for Envoy's gh CLI listener (generated in Dockerfile)
_TLS_CERT = Path("/etc/certs/localhost.crt").read_text() if Path("/etc/certs/localhost.crt").exists() else ""
_TLS_KEY = Path("/etc/certs/localhost.key").read_text() if Path("/etc/certs/localhost.key").exists() else ""
...
class SandboxManager:
    ...
    def _create_envoy_configmap(
        ...
        # Format TLS cert/key for YAML inline_string block (18-space indent)
        _indent = " " * 18
        tls_cert_indented = ("\n" + _indent).join(_TLS_CERT.strip().splitlines()) if _TLS_CERT else ""
        tls_key_indented = ("\n" + _indent).join(_TLS_KEY.strip().splitlines()) if _TLS_KEY else ""
...
static_resources:
  ...
  # TLS listener for gh CLI (which requires HTTPS for API calls)
  # Routes /api/v3/* → credential-resolver /github/* (GitHub Enterprise path convention)
  # gh sends Authorization: token <SANDBOX_JWT>, credential-resolver validates JWT
  # and swaps it for the real GitHub token before proxying to api.github.com.
  - name: gh_tls_proxy
    address:
      socket_address:
        address: 127.0.0.1
            ...
            - name: github_enterprise
              ...
              - match:
                ...
                route:
                  ...
                  prefix_rewrite: "/github/"
                  timeout: 30s
              - match:
                  prefix: "/api/graphql"
                route:
                  cluster: credential_resolver_github
                  prefix_rewrite: "/github/graphql"
                  timeout: 30s
          http_filters:
          - name: envoy.filters.http.router
            typed_config:
  ...
  # GitHub API proxy cluster — used by gh CLI via TLS listener (port 8443)
                        ...
                        "containers": [
                            # Main agent container - NO SECRETS, only proxy config
                            {
                                ...
                                + [
                                    ...
                                    # GH_HOST tells gh to use localhost:8443 as its API endpoint
                                    {
                                        "name": "GH_HOST",
                            ...
                            # Envoy sidecar - handles credential injection via ext_authz
                            {
                                ...
                                "ports": [
                                    ...
                                    {"containerPort": 8443, "name": "gh-proxy"},
                                ],
                                "volumeMounts": [
                                    {
                                        "name": "envoy-config",
