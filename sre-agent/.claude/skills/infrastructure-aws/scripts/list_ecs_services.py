...
def main():
    ...
    try:
        ...
        if args.json:
            print(format_output({"cluster": args.cluster, "count": len(services), "services": services}))
        else:
            print(f"Cluster: {args.cluster}")
            print(f"Services: {len(services)}")
            print()
            print(f"{'NAME':<30} {'STATUS':<10} {'DESIRED':>8} {'RUNNING':>8} {'PENDING':>8} {'LAUNCH TYPE':<12} {'TASK DEF':<30}")
            print("-" * 106)
            for svc in services:
                print(
