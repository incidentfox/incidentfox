...
def main():
    ...
    try:
        ...
        diff = gitlab_request("GET", f"projects/{proj}/repository/commits/{args.sha}/diff", params={"per_page": 100})

        files = [
            {"old_path": d.get("old_path"), "new_path": d.get("new_path"),
             "new_file": d.get("new_file"), "deleted_file": d.get("deleted_file"),
             "diff": d.get("diff", "")[:1000]}
            for d in (diff or [])[:100]
        ]

        result = {
            "ok": True, "id": commit["id"], "short_id": commit["short_id"],
            "title": commit["title"], "message": commit.get("message"),
            "author_name": commit["author_name"], "author_email": commit.get("author_email"),
            "created_at": commit["created_at"], "web_url": commit.get("web_url"),
            "parent_ids": commit.get("parent_ids", []),
            "stats": commit.get("stats", {}), "files_changed": files,
        }
        ...
        if args.json:
            ...
            if stats:
                print(f"Changes: +{stats.get('additions', 0)} -{stats.get('deletions', 0)}")
            if files:
                print(f"\nFiles ({len(files)}):")
                for f in files:
