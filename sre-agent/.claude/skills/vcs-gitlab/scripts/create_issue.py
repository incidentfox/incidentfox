...
def main():
    ...
    try:
        ...
        if args.assignee_ids:
            issue_data["assignee_ids"] = [int(x.strip()) for x in args.assignee_ids.split(",")]

        issue = gitlab_request("POST", f"projects/{encode_project(args.project)}/issues", json_body=issue_data)
        result = {"ok": True, "iid": issue["iid"], "title": issue["title"], "web_url": issue["web_url"]}
