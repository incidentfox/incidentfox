...
def main():
    ...
    try:
        mr = gitlab_request("GET", f"projects/{encode_project(args.project)}/merge_requests/{args.mr_iid}")

        result = {
            "ok": True, "iid": mr["iid"], "title": mr["title"], "description": mr.get("description"),
            ...
            "labels": mr.get("labels", []), "web_url": mr["web_url"],
            "merged_at": mr.get("merged_at"), "merge_status": mr.get("merge_status"),
            "has_conflicts": mr.get("has_conflicts"), "changes_count": mr.get("changes_count"),
        }
        ...
        if args.json:
            ...
            print(f"State: {result['state']} | {result['source_branch']} -> {result['target_branch']}")
