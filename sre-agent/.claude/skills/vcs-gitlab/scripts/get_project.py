...
def main():
    ...
    try:
        ...
        result = {
            "ok": True, "id": p["id"], "name": p["name"], "path": p["path_with_namespace"],
            "description": p.get("description"), "web_url": p["web_url"],
            "default_branch": p.get("default_branch"), "visibility": p.get("visibility"),
            "created_at": p.get("created_at"), "last_activity_at": p.get("last_activity_at"),
            "forks_count": p.get("forks_count"), "star_count": p.get("star_count"),
        ...
        if args.json:
            ...
            print(f"Branch: {result.get('default_branch', '?')} | Visibility: {result.get('visibility', '?')}")
            print(f"Stars: {result.get('star_count', 0)} | Forks: {result.get('forks_count', 0)} | Issues: {result.get('open_issues_count', 0)}")
