...
def main():
    ...
    parser.add_argument("--status", default="", help="Filter: running, pending, success, failed, canceled, skipped")
    ...
    try:
        ...
        data = gitlab_request("GET", f"projects/{encode_project(args.project)}/pipelines", params=params)
        pipelines = [
            {"id": p["id"], "status": p["status"], "ref": p["ref"], "sha": p["sha"],
             "web_url": p["web_url"], "created_at": p.get("created_at"), "source": p.get("source")}
            for p in data
        ]
        ...
        if args.json:
            ...
            for p in pipelines:
                print(f"  [{p['status'].upper():8s}] #{p['id']} | {p['ref']} | {p['sha'][:8]}")
