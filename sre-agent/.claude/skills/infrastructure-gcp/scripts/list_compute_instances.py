...
def main():
    ...
    parser.add_argument("--zone", help="Zone filter (e.g., us-central1-a). Default: all zones")
    ...
    try:
        ...
        if args.zone:
            result = compute.instances().list(project=project_id, zone=args.zone).execute()
            ...
            for zone_data in zones_result.get("items", []):
                ...
                result = compute.instances().list(project=project_id, zone=zone_name).execute()
                for instance in result.get("items", []):
...
def _parse_instance(instance: dict, zone: str) -> dict:
    return {
        ...
        "internal_ip": (
            ...
            if instance.get("networkInterfaces") else None
        ),
        "external_ip": (
