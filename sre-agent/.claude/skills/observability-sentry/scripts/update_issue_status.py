...
def main():
    ...
    parser.add_argument("--status", required=True, choices=["resolved", "unresolved", "ignored", "resolvedInNextRelease"], help="New status")
    parser.add_argument("--json", action="store_true", help="Output as JSON")
    args = parser.parse_args()

    try:
        sentry_request("PUT", f"issues/{args.issue_id}/", json_body={"status": args.status})
