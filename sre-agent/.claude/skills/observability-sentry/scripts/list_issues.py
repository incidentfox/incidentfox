...
def main():
    ...
    parser.add_argument("--limit", type=int, default=25, help="Max issues (default: 25)")
    ...
    try:
        ...
        issues = [
            {
                "id": i["id"], "title": i["title"], "short_id": i["shortId"],
                "status": i["status"], "level": i["level"], "count": i["count"],
                "user_count": i["userCount"], "first_seen": i["firstSeen"],
                "last_seen": i["lastSeen"], "permalink": i["permalink"],
            }
            for i in data
        ]

        result = {"ok": True, "project": project, "issues": issues, "count": len(issues)}
        ...
        if args.json:
            ...
            for issue in issues:
                print(f"  [{issue['level'].upper()}] #{issue['short_id']} - {issue['title']}")
                print(f"    Events: {issue['count']} | Users: {issue['user_count']} | Status: {issue['status']}")
                print()
