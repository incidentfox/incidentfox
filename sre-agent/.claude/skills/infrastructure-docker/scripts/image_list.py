...
def main():
    ...
    docker_args = ["images", "--format", "{{.Repository}}|{{.Tag}}|{{.ID}}|{{.Size}}|{{.CreatedAt}}"]
    if args.filter:
        docker_args.extend(["--filter", args.filter])

    result = run_docker(docker_args)
    if result.get("ok"):
        result["images"] = parse_pipe_delimited(result.get("stdout", ""), ["repository", "tag", "id", "size", "created"])
    ...
    if args.json:
        ...
        for img in result.get("images", []):
            print(f"{img['repository']:40s} {img['tag']:15s} {img['size']:10s} {img['id']:15s}")
