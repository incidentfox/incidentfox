...
flowchart TB
    subgraph External["External Systems"]
        ...
        ExtAPIs["External APIs<br/>Datadog, Grafana,<br/>Coralogix, Snowflake,<br/>Kubernetes, AWS"]
    end

    subgraph VendorInfra["IncidentFox Vendor Infrastructure"]
        VendorService["Vendor Service<br/>(AWS Lambda)<br/>─────────────<br/>• License validation<br/>• Telemetry collection"]
    end

    subgraph CustomerAWS["Customer AWS Infrastructure (On-Prem Deployment)"]
        ...
        subgraph EKS["EKS Cluster (incidentfox namespace)"]
            ...
            subgraph Observability["Observability"]
                Telemetry["Telemetry<br/>Collector<br/>─────────────<br/>• Usage metrics<br/>• License validation<br/>• Opt-out support"]
            end
    ...
    %% Telemetry flows (outbound to vendor)
    Telemetry -->|Read metrics| RDS
    Telemetry -->|License validation<br/>& telemetry| VendorService
...
**Purpose:** License validation and optional usage metrics collection.

| Aspect | Details |
|--------|---------|
| **License Validation** | Validates on-prem deployment license with Vendor Service |
...
### 8. Vendor Service (IncidentFox-Hosted)

**Purpose:** License validation and telemetry collection for on-prem deployments.

| Aspect | Details |
|--------|---------|
| **Hosting** | IncidentFox AWS infrastructure (separate from customer deployment) |
| **License Validation** | Validates customer license keys for on-prem deployments |
| **Telemetry Collection** | Receives aggregate usage metrics from opted-in organizations |
| **Endpoints** | `POST /validate-license`, `POST /telemetry/heartbeat`, `POST /telemetry/analytics` |
| **Security** | TLS encryption, license key authentication |

### 9. Web UI

**Purpose:** Admin console and team configuration interface for end users.

| Aspect | Details |
|--------|---------|
| **Technology** | Next.js |
| **Access** | End users access via web browser (HTTPS) |
...
| Lambda | Vendor Service - hosted by IncidentFox for license validation and telemetry (not in customer VPC) |
